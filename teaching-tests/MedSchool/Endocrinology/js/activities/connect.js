window.ActivityModules = window.ActivityModules || {};
    ...
    for (const [leftBtn, { rightBtn }] of connections) {
        const item = items.find(
            i => i.left === leftBtn.textContent
        );

        if (!item || item.right !== rightBtn.textContent) {
            correct = false;
        }
    }
    ...
    if (correct && connections.size === items.length) {
        ...
        setTimeout(() => {
            container.innerHTML =
                '<div class="activity-complete">Connect activity completed ✅</div>';

            if (
                window.ActivityUtils &&
                window.ActivityUtils.unlockNext
            ) {
                window.ActivityUtils.unlockNext(container);
            }
        }, 700);

    } else {

        // Show solution button after first failed attempt
        if (!hasSolution) {
            hasSolution = true;
        ...
        feedback.textContent =
            '❌ Some connections are incorrect. Retry.';

        for (const l of Array.from(connections.keys())) {
            removeConnection(l);
        }
    }
	});
            ...
            clearBtn.textContent = 'Clear Connections';
            ...
            // --- Solution button (added after first failed try) ---
...
solutionBtn.addEventListener('click', () => {
    ...
    // draw correct connections
    items.forEach(item => {

        const leftBtn = leftButtons.find(
            b => b.textContent === item.left
        );

        const rightBtn = rightButtons.find(
            b => b.textContent === item.right
        );
    ...
    feedback.textContent = '✅ Correct solution shown.';
});
        };