...
export function renderMarkdown(mdext) {
  ...
  // --------------------------------------------------
  // Block placeholder protection (NEW)
  // --------------------------------------------------
  const blockPlaceholders = [];
  function protectBlock(htmlBlock) {
    const key = `%%BLOCK${blockPlaceholders.length}%%`;
  ...
  html = html.replace(
    /^(={2,})\s*(.+?)\s*=*\s*$/gm,
    (_, eqs, title) => `<h${eqs.length}>${title.trim()}</h${eqs.length}>`
  );
  ...
  // Activity containers
  html = html.replace(
    /\[activity:(\w+)(?::id:(\w+))?\]([\s\S]*?)\[end activity\]/g,
    (_, type, id, content) =>
      protectBlock(
        `<div class="activity-container" data-activity="${type}" ${
          id ? `id="${id}"` : ''
        }>${content.trim()}</div>`
      )
  );
  ...
  // --------------------------------------------------
  // Tables (unchanged logic, but protected)
  // --------------------------------------------------
  html = html.replace(/((?:\|.+\|\s*\n)+)/g, (match) => {
    ...
    return protectBlock(`<table>${thead}<tbody>${tbodyRows}</tbody></table>`);
  });

  // --------------------------------------------------
  // Lists (ul, ol, dl) (FIXED)
  // --------------------------------------------------
  ...
  html = html.replace(
    ...
    (_, block) => {
      const items = block.trim().split('\n').map(line => {
        ...
        return `<dt>${term.trim()}</dt><dd>${def.trim()}</dd>`;
      }).join('');
      return protectBlock(`<dl>${items}</dl>`);
    }
  );
  ...
  html = html.replace(
    ...
    (_, block) => {
      ...
      return protectBlock(`<ul>${items}</ul>`);
    }
  );
  ...
  html = html.replace(
    ...
    (_, block) => {
      ...
      return protectBlock(`<ol>${items}</ol>`);
    }
  );
  ...
  // --------------------------------------------------
  // Newlines → <br> (unchanged behavior)
  // --------------------------------------------------
  html = html.replace(/\n/g, '<br>');

  // --------------------------------------------------
  // Restore protected block HTML (NEW)
  // --------------------------------------------------
  blockPlaceholders.forEach((block, i) => {
    html = html.replace(`%%BLOCK${i}%%`, block);
  });
  
  // --- Clean up <br> around block-level elements (lists, tables, activities) ---
  html = html.replace(/(<br>\s*)+(<(?:ul|ol|dl|table|div)\b)/g, '$2');
  html = html.replace(/(<\/(?:ul|ol|dl|table|div)>\s*)(<br>)+/g, '$1');
